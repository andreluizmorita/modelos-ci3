...
class CI_Assets
{
	...
	public function __construct()
	{
		...
		$this->base_url = str_replace('[::1]','localhost',$this->CI->config->item('base_url'));

		$this->img_folder = $this->base_url . $this->CI->config->item('asset_path') . $this->CI->config->item('img_folder');
		$this->js_folder  = $this->base_url . $this->CI->config->item('asset_path') . $this->CI->config->item('js_folder');
		$this->css_folder = $this->base_url . $this->CI->config->item('asset_path') . $this->CI->config->item('css_folder');
	...
	protected function Stylesheet($asset_name, $media , $return_href )
	{
		...
		if ( $asset_name != NULL ):		
			...
			{
				if($return_href === true)
					return $this->base_url . $this->CI->config->item('asset_path') . $asset_name . '.css';
				
				$stylesheet .= ' href="' . $this->base_url . $this->CI->config->item('asset_path') . $asset_name . '.css" />';
			}
