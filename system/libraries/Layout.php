...
class CI_Layout
{
	...
	# --- Subpasta usada no controller e onde irá buscar os layouts referentes
	...
	{  
        ...
        if(file_exists( APPPATH . 'views/'. $this->folder . '_modules/'. $module .'.php'))
        {
            $this->debug_module('Module', $this->folder . '_modules/'. $module . '.php');
        ...
        else
        {
            ...
            $this->debug_module('Module', $this->folder . '_modules/'. $module . '.php',FALSE);
        }
	...
	{
		$this->set_layout_class = $name . '.php';
	}
	...
	{
		$this->set_layout_method= $name . '.php';
	}
	...
	{	
		$this->folder = (is_null($this->subfolder))?$this->folder:$this->folder.'/'.$this->subfolder.'/';

		// --- Define o layout principal
		$this->config['main'] = isset($this->set_layout_main)? $this->set_layout_main : $this->folder . $this->layout_main . '.php';
        $this->config['main_class'] = isset($this->set_layout_main)? $this->set_layout_main : $this->folder . $this->layout_main . '_class' . '.php';
		...
		$this->config['layout_main']  = $this->folder  . $this->class . '/'. $this->layout_main . '.php';
        $this->config['layout_main_class']  = $this->folder  . $this->class . '/'. $this->layout_main . '_class' . '.php';
		// --- Define layout padrão de uso para os controllers que extenderem essa classe. Se o arquivo não existir não será carregado
		$this->config['layout_class'] = isset($this->set_layout_class)? $this->set_layout_class : $this->folder . $this->class . '/' . $this->class . '.php';
		// --- Define layout padrão de uso para os controllers conforme o método usada. Se o arquivo não existir não será carregado
		$this->config['layout_method'] = isset($this->set_layout_method)? $this->set_layout_method : $this->folder . $this->class . '/' . $this->method . '.php';
		// --- Define layout simple onde é substituida as variáveis parse se existirem
		$this->config['layout_main_page']   = $this->folder . $this->class . '/' . $this->layout_main . '_' . $this->method . '.php';
     ...
     * @access  public 
     * 
	 * @param string $subfolder Nome da pasta onde os layouts serão procurados.
	 * @param boolean $clear Define se o títulos será concatenado com anterior. Quando falso concatena com string definidas antes.
	 */
	public function subfolder( $subfolder , $clear = FALSE )
	{
		if($clear)
